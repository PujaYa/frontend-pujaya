...
const AuctionInfoBox: React.FC<AuctionInfoBoxProps> = ({
  ...
  const currentPrice = auctionInfo?.initialPrice ?? initialPrice;
  return (
    <div className="bg-white rounded-2xl shadow p-6 mb-2 flex flex-col items-center">
      ...
      <h2 className="text-xl font-bold text-gray-900 mb-2 leading-tight text-center w-full">
        {auctionInfo?.name}
        {/*
        // Botones de edición eliminados porque isOwner, auctionId, id no están definidos
        {isOwner && (
      ...
      <div className="flex gap-2 mb-4">
        <span
          className={`px-3 py-1 rounded-full text-xs font-semibold ${
            auctionInfo?.isActive ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-500'
          }`}
        >
          {auctionInfo?.isActive ? 'Active' : 'Finished'}
        </span>
      </div>
      {/* 2. Category badge */}
